...
class ConditionalMappingNetwork(nn.Module):
    ...
    def __init__(self, z_dim: int, w_dim: int, num_classes: int = 2, num_layers: int = 8, embed_dim: int = 128):
        """
        Initializes the ConditionalMappingNetwork.

        Following official StyleGAN2 approach: no normalization in mapping network,
        smaller class embeddings (128 instead of 512), and proper w_avg tracking.

        Args:
            z_dim (int): Dimension of the input latent vector z.
            w_dim (int): Dimension of the output intermediate latent vector w.
            num_classes (int, optional): Number of class categories. Defaults to 2.
            num_layers (int, optional): Number of FC layers. Defaults to 8.
            embed_dim (int, optional): Dimension of class embedding. Defaults to 128.
        """
        ...
        # Smaller class embedding (128 is sufficient for 2 classes, not 512)
        # Official StyleGAN2 uses embedding_dim << w_dim
        self.class_embed = nn.Embedding(num_classes, embed_dim)
        
        # Build mapping network: [z, class_embed] -> w
        layers = []
        for i in range(num_layers):
            in_dim = z_dim + embed_dim if i == 0 else w_dim
            layers.append(EqualizedLinear(in_dim, w_dim))
            layers.append(nn.LeakyReLU(0.2))
        
        self.mapping = nn.Sequential(*layers)
        
        # Track running average of w for truncation trick (official StyleGAN2 feature)
        self.register_buffer('w_avg', torch.zeros(w_dim))
    
    def forward(self, z: torch.Tensor, class_labels: torch.Tensor, 
                truncation_psi: float = 1.0, update_w_avg: bool = True) -> torch.Tensor:
        """
        Forward pass through the conditional mapping network.
        
        Simplified from overcomplicated version - follows official StyleGAN2:
        - NO z normalization (network learns proper scales)
        - Direct concatenation of z and class embedding
        - Proper w_avg tracking for truncation trick

        Args:
            z (torch.Tensor): Input latent vector z of shape [batch_size, z_dim].
            class_labels (torch.Tensor): Class labels of shape [batch_size].
            truncation_psi (float): Truncation factor (1.0 = no truncation).
            update_w_avg (bool): Whether to update w_avg running mean.

        Returns:
            torch.Tensor: Output w vector of shape [batch_size, w_dim].
        """
        # Validate inputs
        ...
        # Get class embedding and concatenate with z
        # NO normalization - official StyleGAN2 doesn't normalize in mapping network
        ...
        # Update running average of w during training (for truncation trick)
        if update_w_avg and self.training:
            with torch.no_grad():
                batch_avg = w.mean(dim=0)
                self.w_avg.copy_(self.w_avg * 0.995 + batch_avg * 0.005)
        
        # Apply truncation trick if psi < 1.0 (official StyleGAN2 feature)
...
class Generator(nn.Module):
    ...
    def __init__(self, log_res: int, w_dim: int, base_channels: int = 64, max_channels: int = 512):
        ...
        # Calculate number of style injection points (official StyleGAN2 formula)
        # One for initial block, then two per upsampling block
        self.num_layers = 2 * (log_res - 1)  # e.g., 2 * (8-1) = 14 for 256x256
...
class Conv2dWeightModulate(nn.Module):
    ...
    Official StyleGAN2 implementation - no artificial clamps.
    """
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, 
        ...
        Args:
            ...
            demodulate (bool, optional): Whether to apply weight demodulation. Defaults to True.
    ...
    def forward(self, x: torch.Tensor, style: torch.Tensor) -> torch.Tensor:
        """
        Forward pass with weight modulation (official StyleGAN2 approach).
        
        No artificial clamps - proper training balance prevents extreme values.

        Args:
            x (torch.Tensor): Input feature map of shape [batch, in_channels, H, W].
            style (torch.Tensor): Style vector of shape [batch, in_channels].
        ...
        # Add 1 to style so it acts as multiplicative modulation centered at 1
        # Official StyleGAN2: weight * (style + 1)
        style = style.view(batch, 1, in_c, 1, 1)
        weight = self.weight()[None] * (style + 1)

        # Demodulation (official StyleGAN2 - normalizes output variance)
        if self.demodulate:
            # Calculate per-filter standard deviation
            demod = torch.rsqrt((weight ** 2).sum(dim=(2, 3, 4), keepdim=True) + self.eps)
            # Normalize weights
            weight = weight * demod

        # Reshape for grouped convolution (efficient batch processing)
        x = x.reshape(1, batch * in_c, h, w)
...
class EMA:
    ...
    Official StyleGAN2 uses EMA generator for evaluation, which produces
    higher quality and more stable outputs than the raw training generator.
    """
    def __init__(self, model: nn.Module, decay: float = 0.999):
        ...
        Args:
            model (nn.Module): Model to track (typically the generator).
            decay (float): Decay rate for moving average. Defaults to 0.999.
        """
        self.model = model
        self.decay = decay
        self.shadow = {}
    ...
    def apply_shadow(self):
        """
        Replace model parameters with their EMA values.
        Use before evaluation/inference.
        """
        for name, param in self.model.named_parameters():
            if param.requires_grad:
    ...
    def restore(self):
        ...
        Use after evaluation to resume training.
        """
        for name, param in self.model.named_parameters():
            if param.requires_grad:
